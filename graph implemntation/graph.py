...
def has_path(graph, src, dst):
  """
  checks if dst is approchable starting from src in given acyclic graph
  """
  if dst in graph[src]:
    return True
  res = False
  for neighbour in graph[src]:
    res = res or has_path(graph, neighbour, dst)
  return res
...
def has_path(graph, src, dst, visited):
    ...
    for neighbour in graph[src]:
        if neighbour not in visited:
            res = res or has_path(graph, neighbour, dst, visited)
    return res
